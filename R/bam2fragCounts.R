...
Bam2FragCounts <- 
  ...
  if (addToName != "")
    seqnms <- sub(addToName,'',GenomeInfoDb::seqlevels(GATC.mpbl.reads))
  else
    seqnms <- GenomeInfoDb::seqlevels(GATC.mpbl.reads)
  rngs <- GenomicRanges::GRanges(seqnms, IRanges::IRanges(1, GenomeInfoDb::seqlengths(GATC.mpbl.reads)))
